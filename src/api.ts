...
export async function api<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(payload.detail || payload.error?.message || 'Something went wrong', response.status)
  }
...
export const shortDate = (value?: string | null) =>
  value ? new Intl.DateTimeFormat('en-ZM', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(value)) : '—'
